...
function invalidShas(results,test){
...
function TestDefaultHashes(){
	...
	[
		...
		Yes(['sha256']),
		Yes(['sha256','sha512'])
	].forEach((item) => (item) ? failTest('defaultHashes: '+JSON.stringify(item)) : null);
...
function TestProgrammaticResources(){
	...
	.then((results) => {
		...
		invalidShas(results,'programmatic')
...
function TestCommandLineResources(){
	...
	require('child_process').exec(
		...
		(error,stdout,stderr) => {
			...
			invalidShas(stdout.results,'CLI');
